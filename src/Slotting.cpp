...
void DirectionalDrillingSolver::setData(const std::string& filename)
{
	...
	for (auto y : jf["Constraints"]["Layers"])
	{
		...
		if (!isnan)
		{
			constraints.cs.push_back(Constraint{
				{(y["TVD"]), (y["inc"][0]), (y["azi"][0])},
				{(y["TVD"]), (y["inc"][1]), (y["azi"][1])}});
		}
	}
...
std::vector<TrajectoryTemplate*> DirectionalDrillingSolver::wellfunction(
    ...
    tmp.push_back(new CurveHoldCurveHold(tmp.back()->pointT1, 0, 0, R1, R2,
                                            target, x[3], x[4], x[5]));
	return tmp;
}
...
void DirectionalDrillingSolver::Optimize()
{
	...
	std::function<double(const std::vector<double>&)> score =
            [&](const std::vector<double>& x)
	{
        ...
        double penLen =
            abs(constraints.wc.maxMD - 1 / EPSILON) < EPSILON
                ? 0 : PenaltyLength(allLength(tmp), constraints.wc.maxMD,20);
        double penConstraints =
            constraints.cs.size() == 0
                ? 0 : PenaltyConstraint(constraints.cs, pCtmp, pMDtmp, 100);
	...
	for (size_t i = 0; i < targets.size(); ++i)
	{
		...
		{
			opt = PSO(score, minValues, maxValues, 3 * minValues.size(),150,
					  std::vector<double>(150, 0.7298), 1.49618, 1.49618);
...
void DirectionalDrillingSolver::writeResults(const std::string& outputdirpath)
{
