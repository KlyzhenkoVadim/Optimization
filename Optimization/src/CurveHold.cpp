...
CurveHold::CurveHold(const Eigen::Vector3d& p1, const Eigen::Vector3d& p3,
                     double teta, double phi, double R, size_t nums) {
    this->p1 = p1;
    this->p3 = p3;
    this->teta = teta;
    this->phi = phi;
...
CurveHold::CurveHold(const Eigen::Vector3d& p1, const Eigen::Vector3d& p3,
                     const Eigen::Vector3d& t1, double R, size_t nums) {
    this->p1 = p1;
    this->p3 = p3;
    this->t1 = t1;
    this->R = R;
...
double CurveHold::length() {
    // длина шаблона - длина дуги + длина участка стабилизации.
    double arc = alpha * R;
    return arc + betta;
}
...
int CurveHold::fit() {
    Eigen::Vector3d b1 = (p3 - p1).cross(t1); // вектор плоскости, в которой лежит шаблон.
    Eigen::Vector3d n1 = t1.cross(b1); // вектор направленной от дуги к её центру
    n1.normalize();
    Eigen::Vector3d r = p1 + R * n1; // положение центра окружности радуса R
    double norm = (p3 - r).norm();
    if (b1.norm() < EPSILON) {
        // случай, когда дуга нулевой длины - просто отрезок
        t2 = t1;
        alpha = 0;
        betta = (p3 - p1).norm();
    }
    ...
    {
        if ((norm - R) < -EPSILON)  // проверка того, что расстояние от центра
                                    // окружности до начальной точки меньше R
        {
            return -1;
        }
        // Построение описано в основной статье Sawaryn et al. 2005
        // Обозначения тоже оттуда.
        ...
        // Отдельно рассмотрен случай при нулевом знаменателе формулы (1)
        if (fabs(2 * R - ksi) < EPSILON) {
            alpha = 2 * atan(0.5 * ksi / etta);
        }
        else {
            alpha = 2 * atan((etta - betta) / (2 * R - ksi)); //(1)
        }

        t2 = (p3 - p1 - R * tan(alpha / 2.0) * t1) / (betta + R * tan(alpha / 2.0));
        t2.normalize();
        // Зануление незначащих координат.
        for (auto& it : t2) {
            if (fabs(it) < EPSILON) {
                it = 0.0;
            }
...
Eigen::Vector3d CurveHold::FunctionPoint(double md) // md [0,1]
{
    ...
    // Проверка, точка md лежит на дуге или отрезке.
    if (s - 1 > EPSILON)
    {
        if (betta < EPSILON)
            return p3;
...
void CurveHold::points(CoordinateSystem coordinateSystem) {
    ...
    double arc = alpha * R;
    Eigen::Vector3d pArcEnd = p3 - betta * t2; // TODO: getPointInterpol()

    size_t num = std::max(10, int(arc / h)); // число точек выделенное на дугу
    size_t stepHold = std::max(2, int(betta / h)); // -//- на отрезок, пропорционально длинам.
    // Интерполируется сначала дуга, затем отрезок и результат объединяется.
    if (coordinateSystem == CoordinateSystem::CARTESIAN) {
